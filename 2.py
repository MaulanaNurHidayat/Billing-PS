    ...
    if pilihan == "1":
        ...
        for nama, data in pelanggan_aktif.items():
            if data["waktu_berakhir"] is None:
                ...
                print(f"- {nama} (Per Jam)")

        nama_pelanggan = input("Masukkan nama pelanggan: ").upper()
        if nama_pelanggan not in pelanggan_aktif:
            print("Nama pelanggan tidak ditemukan.")
            continue

        data = pelanggan_aktif[nama_pelanggan]
        if data["waktu_berakhir"] is None:
            print(f"Pelanggan '{nama_pelanggan}' menggunakan sistem unlimited.")
        else:
            waktu_sisa = max(0, int(data["waktu_berakhir"] - time.time()))
            jam, sisa_detik = divmod(waktu_sisa, 3600)
            menit, detik = divmod(sisa_detik, 60)
            print(f"\nSisa waktu untuk pelanggan '{nama_pelanggan}': {jam} jam {menit} menit {detik} detik")
        ...
        with open("laporan_harian.txt", "a") as file:
            file.write(
                ...
                f"Biaya yang harus dibayar: Rp{biaya:,}\n"
                f"{'-'*50}\n"
            )
